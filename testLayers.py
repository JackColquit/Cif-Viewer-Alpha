...
def convertDataToColour(the2dReal,maxRealValue,theCut2):
    ...
    while i<theCut2:
        j=0
        while j<theCut2:
            imagedata1=int((the2dReal[i,j]*255)/maxRealValue)
...
def twoDimensionArrayPotential(anotherOneLambda,subArray,elementsInUseData,big2,secondZ,lastZ):
    ...
    for barrier in subArray:
        ...
        lastX2 = float(barrier.showX())
        lastY2 = float(barrier.showY())
        ...
        subChemicalIndex=int((lastX2+big2)%lastY2)
        preDone2[subChemicalIndex]=electronicPotential
    return preDone2
#---------------------End electronic Potential Library-------------------
#---------------------Start Propagator Library---------------------------
#this function only for wave propagator
def propagator(anotherLambda3,subArray2,big):
    preDone=[]
    biggy=big*big
    for pad in range(biggy):
        preDone.append(0)
    for wave in subArray2:
        lastX = float(wave.showX())
        lastY = float(wave.showY())
        ...
        subPropagatorIndex=int((lastX+big)%lastY)
        preDone[subPropagatorIndex]=charge5
    return preDone
def oneLayer(chemicalPotential,eTransmission,psiSignal,side):
    x1 = np.array( psiSignal, dtype = complex )
    x2 = np.reshape(x1,(side,side))
    y1 = np.array( eTransmission, dtype = complex )
    y2 = np.reshape(y1,(side,side))
    z1 = np.array( chemicalPotential, dtype = complex )
    z2 = np.reshape(z1,(side,side))
    moc1=np.dot(y2,x2)
...
blackSheep=findLambda(80000)
#extract all data
allText=generalExtractor(pathTest)
#first the elements to check
...
preImage2=np.fft.fftshift(preImage1)
preImage3=abs(preImage2)
maxRealColour=np.amax(preImage3)
Image4=convertDataToColour(preImage3,maxRealColour,nextOne)
nameImage="Plot "+pathTest
fig = plt.figure(figsize=(nextOne,nextOne))
plt.pcolormesh(Image4,cmap="plasma")
plt.title(nameImage)
plt.colorbar()
plt.show()
